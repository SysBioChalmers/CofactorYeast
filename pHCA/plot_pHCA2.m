...
gname_1 = [gname_1;{'pHCA pathway'}];
gname_2 = [gname_2;{'pHCA pathway'}];
% extend CofactorDataset
[~,gprlst,~] = xlsread('New_pathway_pHCA.xlsx','Gpr_info');
[gpr_cfclst,~,~] = xlsread('New_pathway_pHCA.xlsx','Gpr_info');
...
plot(mu_fixed_fe,pHCA_fixed_fe,'-o','MarkerSize',2,'LineWidth',0.75,'Color','k');
xlim([0 0.4]);
% ylim([0 0.1]);
set(gca,'FontSize',6,'FontName','Helvetica');
xlabel('Growth rate (/h)','FontSize',7,'FontName','Helvetica');
ylabel(['pHCA production rate',char(13,10)','(mmol/gCDW/h)'],'FontSize',7,'FontName','Helvetica');
...
% for fixed mu
fluxes_fixed_mu = spHCA2_res.fluxes(:,round(spHCA2_res.fluxes(strcmp(model.rxns,'r_2111'),:),3) == fixed_mu);
fe_fixed_mu = -fluxes_fixed_mu(strcmp(model.rxns,'r_1861'),:);
ub_fe_fixed_mu = -spHCA2_res.record(:,round(spHCA2_res.fluxes(strcmp(model.rxns,'r_2111'),:),3) == fixed_mu);
mu_fixed_mu = fluxes_fixed_mu(strcmp(model.rxns,'r_2111'),:);
Noscapine_fixed_mu = fluxes_fixed_mu(strcmp(model.rxns,'new_r_HCAex'),:);
