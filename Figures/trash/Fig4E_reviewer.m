...
load('sIRCAA_res.mat');
load('sIRCS_res.mat');
fluxes = [sIRCAA_res.fluxes sIRCS_res.fluxes(:,2:end)];
labels = [sIRCAA_res.lables sIRCS_res.lables(:,2:end)];
...
for i = 1:length(AA_IDs)
    fluxes_tmp = fluxes(:,ismember(labels,AA_IDs(i)));
...
load('sRRCAA_res.mat');
load('sRRCS_res.mat');
fluxes = [sRRCAA_res.fluxes sRRCS_res.fluxes(:,2:end)];
lables = [sRRCAA_res.lables sRRCS_res.lables(:,2:end)];
mu = fluxes(ismember(model.rxns,'r_2111'),:);
delta_mu = mu(2:end)-mu(1);
delta_aa = zeros(1,length(AA_IDs));
for i = 1:length(AA_IDs)
    fluxes_tmp = fluxes(:,ismember(lables,AA_IDs(i)));
    delta_aa(1,i) = abs(fluxes_tmp(ismember(model.rxns,AA_Exchanges{i}),:));
end
...
figure('Name','1');
h = heatmap(lables(2:end),{'Unlimited iron uptake';'50% iron uptake'},data,'Colormap',clrmap,...
