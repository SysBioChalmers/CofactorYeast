...
% selected_gene = {'pHCA pathway';'Lys4';'Ole1';'Leu1';'Ilv3';'Cyc1';'Cob';'Rip1'};
% selected_geneID = {'pHCA pathway';'YDR234W';'YGL055W';'YGL009C';'YJR016C';'YJR048W';'Q0105';'YEL024W'};
selected_gene = {'pHCA pathway';'Lys4';'Ole1';'Leu1';'Ilv3';'ETC'};
selected_geneID = {'pHCA pathway';'YDR234W';'YGL055W';'YGL009C';'YJR016C';'ETC'};

q_fe_list = unique(spHCA2_res.record);
tot_raw = sum(spHCA2_res.record == min(q_fe_list));
data_mu = zeros(tot_raw,length(q_fe_list));
data_pHCA = zeros(tot_raw,length(q_fe_list));

for i = 1:length(q_fe_list)
    idx_tmp = spHCA2_res.record == q_fe_list(i);
    flux_tmp = spHCA2_res.fluxes(:,idx_tmp);
    mu = flux_tmp(strcmp(model.rxns,'r_2111'),:);
    glc = -1*flux_tmp(strcmp(model.rxns,'r_1714'),:);
    pHCA = flux_tmp(strcmp(model.rxns,'new_r_HCAex'),:);
%     pHCA = pHCA./glc;
    data_mu(:,i) = [mu nan(1,tot_raw-length(mu))];
    data_pHCA(:,i) = [pHCA nan(1,tot_raw-length(pHCA))];
end

%% 3d plot
figure('Name','1');
surf(-q_fe_list,data_mu,data_pHCA,'EdgeColor','none');
hold on;
grid on;
az = -145;
el = 25;
...
zlabel(['pHCA production rate',char(13,10)','(mmol/gCDW/h)'],'FontSize',7,'FontName','Helvetica');
...
figure('Name','2');
surf(-q_fe_list,data_mu,data_pHCA,'FaceAlpha',0.6,'EdgeColor','none');
hold on;
grid on;
az = -145;
el = 25;
...
% ylim([0 0.4]);
zlim([0 4]);
plot3(-fixed_fe*[1,1],[0,0],[0,4],'k-','LineWidth',1);
plot3(-fixed_fe*[1,1],[0.4,0.4],[0,4],'k-','LineWidth',1);
plot3(-fixed_fe*[1,1],[0.4,0],[0,0],'k-','LineWidth',1);
plot3(-fixed_fe*[1,1],[0.4,0],[4,4],'k-','LineWidth',1);
set(gca,'FontSize',6,'FontName','Helvetica');
xlabel(['UB of iron uptake',char(13,10)','(mmol/gCDW/h)'],'FontSize',7,'FontName','Helvetica');
ylabel('Growth rate (/h)','FontSize',7,'FontName','Helvetica');
zlabel(['pHCA production rate',char(13,10)','(mmol/gCDW/h)'],'FontSize',7,'FontName','Helvetica');
...
ylim([0 0.4]);
zlim([0 4]);
plot3([0,0],fixed_mu*[1,1],[0,4],'k-','LineWidth',1);
plot3([0,4e-4],fixed_mu*[1,1],[0,0],'k-','LineWidth',1);
plot3([4e-4,4e-4],fixed_mu*[1,1],[0,4],'k-','LineWidth',1);
plot3([4e-4,0],fixed_mu*[1,1],[4,4],'k-','LineWidth',1);
set(gca,'FontSize',6,'FontName','Helvetica');
xlabel(['UB of iron uptake',char(13,10)','(mmol/gCDW/h)'],'FontSize',7,'FontName','Helvetica');
ylabel('Growth rate (/h)','FontSize',7,'FontName','Helvetica');
zlabel(['pHCA production rate',char(13,10)','(mmol/gCDW/h)'],'FontSize',7,'FontName','Helvetica');
...
% for fixed fe uptake
fluxes_fixed_fe = spHCA2_res.fluxes(:,spHCA2_res.record == fixed_fe);
fe_fixed_fe = -fluxes_fixed_fe(strcmp(model.rxns,'r_1861'),:);
mu_fixed_fe = fluxes_fixed_fe(strcmp(model.rxns,'r_2111'),:);
pHCA_fixed_fe = fluxes_fixed_fe(strcmp(model.rxns,'new_r_HCAex'),:);

% for fixed mu
fluxes_fixed_mu = spHCA2_res.fluxes(:,round(spHCA2_res.fluxes(strcmp(model.rxns,'r_2111'),:),6) == fixed_mu);
fe_fixed_mu = -fluxes_fixed_mu(strcmp(model.rxns,'r_1861'),:);
ub_fe_fixed_mu = -spHCA2_res.record(:,round(spHCA2_res.fluxes(strcmp(model.rxns,'r_2111'),:),6) == fixed_mu);
mu_fixed_mu = fluxes_fixed_mu(strcmp(model.rxns,'r_2111'),:);
pHCA_fixed_mu = fluxes_fixed_mu(strcmp(model.rxns,'new_r_HCAex'),:);

figure('Name','2d_fixed_fe');
plot(mu_fixed_fe,pHCA_fixed_fe,'-o','MarkerSize',0.5,'LineWidth',0.75,'Color','k');
xlim([0 0.4]);
ylim([0 4]);
set(gca,'FontSize',6,'FontName','Helvetica');
xlabel('Growth rate','FontSize',7,'FontName','Helvetica');
ylabel('pHCA production','FontSize',7,'FontName','Helvetica');
...
plot(ub_fe_fixed_mu,pHCA_fixed_mu,'-o','MarkerSize',0.5,'LineWidth',0.75,'Color','k');
% xlim([0 4e-4]);
ylim([0 4]);
set(gca,'FontSize',6,'FontName','Helvetica');
