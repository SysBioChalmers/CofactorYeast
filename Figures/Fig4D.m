...
figure();
selected = 14:1:length(mu);
data = [mu_50uptake data1(selected) mu_ref];
lbl = ['50% uptake' lbl1(selected)' '100% uptake'];
lbl = strrep(lbl,'_enzyme','');
lbl = cellfun(@(x) strrep(x,'_','-'),lbl,'UniformOutput',false);
b1 = bar(data,0.7,'FaceColor','flat','LineWidth',0.5);
b1.CData(2:end-1,:) = repmat([242,94,13]/255,length(selected),1);
b1.CData(end,:) = [0 0 0];
...
set(gcf,'position',[300 300 120 120]);
set(gca,'position',[0.23 0.4 0.75 0.4]);
