...
circlesize = linspace(10,2,20);
circlesize = repmat(circlesize,1,13);
...
for i = 1:length(ion_id_list)
    ...
    for j = 1:length(conserved_id_list)
        index = find(idx2(:,i) == j);
        h = scatter(score(index,1),score(index,2),circlesize(1,index),'o','filled','LineWidth',1,'MarkerFaceColor',conserved_color(j,:),'MarkerFaceAlpha',alpha(j));legend off;
    end
    xlim([-90 90]);
    ylim([-35 65]);
	set(gca, 'XColor','k');
...
set(gcf,'position',[200 200 380 180]);
