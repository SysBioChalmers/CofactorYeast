...
%% Check if each new metabolite can be produced
...
[~,idx] = ismember(metlst(:,1),model.metNames);
mets_test = model.mets(idx);
[model_newDemand, addedRxns] = addDemandReaction(model, mets_test);
for i = 1:length(mets_test)
    ...
    solTotal(i) = solution.f;
    if abs(solution.f) < optTol
        % unable to be synthesized
        missingMets(m) = mets_test(i);
        m = m + 1;
    else
        % able to be synthesized
        presentMets(p) = mets_test(i);
        p = p + 1;
    end
end
